...
class Core():
  ...
  def drawBars(self, spectrum, image, color):
    ...
    if ADDBARS == 1:
      for j in range(0, 63):
        if ADDTRANSPARENTBAR == 1:
          draw.rectangle((15 + 2 + j * 20, 315 + YOFF, 15 - 2 + j * 20 + 10, YOFF + 325 - (math.pow(spectrum[max(int(j/2),min(255,int(math.pow(j,2)/16)))],2)/255) * 2 - 10), fill=color2) #spectrum[j * 4]
        draw.rectangle((15 + 2 + j * 20, 320 + YOFF, 15 - 2 + j * 20 + 10, YOFF + 320 - (math.pow(spectrum[max(int(j/2),min(255,int(math.pow(j,2)/16)))],2)/255) * 2), fill=color)
